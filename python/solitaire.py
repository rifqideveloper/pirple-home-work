...
class printValid():
    def Commands():
        resume = '--resume'
        input_coment2 = 'press ({}) key to continue'.format(resume)
        while True:
            console.clear()
...
if __name__ == "__main__":
    ...
    game_over = ["Game exited.","Congratulations! You've won!"]
    help__ = 'help__'
    Error = ['no text enter please enter againt',
             "Error! No card could be moved from the Waste to the Foundation.",
             "Error! No card could be moved from the Waste to the Tableau column.",
             "Error! No card could be moved from the Tableau column to the Foundation.",
             "Error! No card could be moved from that Tableau column.",
    ...
    input_coment = "{} Enter a command (type '{}' for help): ".format(player_name,help__)
    while not f.gameWon():
        console.clear()
        print(BREAK_STRING, welcome)
        printTable(t, f, sw)
        command = input(input_coment)
